...
namespace MeowEngine::Rendering {
    GLTransformGizmoPipeline::GLTransformGizmoPipeline() {
    ...
    void GLTransformGizmoPipeline::Draw(const RenderContext& context, const TransformGizmoDrawData& data) const {
        if (!data.Shader.IsValid()) {
            return;
        }

        const auto& shader = context.ResourceManager->GetShaderResource(data.Shader);
        const auto& gizmo = context.ResourceManager->GetGizmoResource(data.Gizmo);

        glUseProgram(shader.GetID());

        glUniformMatrix4fv(glGetUniformLocation(shader.GetID(), "u_view"), 1, GL_FALSE, &data.ViewMatrix[0][0]);
        glUniformMatrix4fv(glGetUniformLocation(shader.GetID(), "u_projection"), 1, GL_FALSE, &data.ProjectionMatrix[0][0]);

        glUniform3fv(glGetUniformLocation(shader.GetID(), "u_cameraPosition"), 1, &data.CameraPosition[0]);
        glUniform3fv(glGetUniformLocation(shader.GetID(), "u_handlePosition"), 1, &data.SelfPosition[0]);
        glUniformMatrix4fv(glGetUniformLocation(shader.GetID(), "u_handleRotation"), 1, GL_FALSE, &data.RotationMatrix[0][0]);

        glUniform1f(glGetUniformLocation(shader.GetID(), "u_handleScale"), 0.11f);
        glUniform1i(glGetUniformLocation(shader.GetID(), "u_selectedAxis"), -1);
        ...
        glBindVertexArray(gizmo.GetVertexArrayObject());
        glDrawElements(GL_TRIANGLES, gizmo.GetNumIndices(), GL_UNSIGNED_INT, 0);
        glBindVertexArray(0);
        ...
        // glUniformMatrix4fv(glGetUniformLocation(ShaderProgramID, "u_handleRotation"), 1, GL_FALSE, &rotationMatrix[0][0]);
        //
    ...
    // void GLTransformGizmoPipeline::CreateHandle(
    //         std::vector<float>& vertices,
    //         std::vector<uint32_t>& indices) {
    //
    ...
    // }
    //
    // void GLTransformGizmoPipeline::AddAxisLines(
    ...
    // }
    //
    // void GLTransformGizmoPipeline::AddScaleCubes(
    //         std::vector<float>& pVertices,
    //         std::vector<uint32_t>& pIndices,
    //         const std::function<uint32_t(glm::vec3, int, int)>& pAddVertex,
    //         int pAxis,
    ...
    // }
    //
    // void GLTransformGizmoPipeline::AddArrowHeads(
    //         std::vector<float>& pVertices,
    //         std::vector<uint32_t>& pIndices,
    //         const std::function<uint32_t(glm::vec3, int, int)>& pAddVertex,
    //         int pAxis,
    ...
    //
    //
    // void GLTransformGizmoPipeline::AddRotationRings(
    //         std::vector<uint32_t>& pIndices,
    //         const std::function<uint32_t(glm::vec3, int, int)>& pAddVertex,
    //         int pAxis
    // ) {
